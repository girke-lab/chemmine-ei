...
int main (int argc, char *argv[])
{
    ...
    desc.add_options()
        ...
        ("coord,C", po::value<string>(&coord_file), "coord file")
    ...
    if (vm.count("help") || (vm.count("data") < 1) || (vm.count("coord") <1))
    {
        cerr << desc;
        return 0;
    }
    ...
    metric::l2sqr<float> l2sqr(data.getDim());
	 ...
	 unsigned int k = K;
	 ifstream inp_f;
	 inp_f.open(coord_file.c_str(), std::ios::in);
	 if (not inp_f.good()) {
	 	std::cerr << "Error in opening coord file\n" << std::endl;
	 	exit(1);
	 }
	 inp_f.getline(inp, BUFSIZE);
	 if (inp_f.fail()) {
	 	std::cerr << "Error in reading coord file\n" << std::endl;
	 	exit(1);
	 }
